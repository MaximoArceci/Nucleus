...
class PacienteCreate(BaseModel):
    ...
    @field_validator("cantFichas", "fichasTotales")
    def validate_cantFichas(cls, value, info: FieldValidationInfo):
        if value < 0:
            raise HTTPException(
                status_code=400, detail=f"El campo {info.field_name} no puede ser negativo")
        return value

    @field_validator("tipoFicha")
    def validate_tipoFicha(cls, value, info: FieldValidationInfo):
        if value not in ["FICHA 1", "FICHA 2", "FICHA 3", "FICHA 4", "FICHA 5", "FICHA 6"]:
            raise HTTPException(
